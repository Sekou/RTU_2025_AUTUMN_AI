...
class Robot:
    def __init__(self, x, y, alpha):
        ...
        self.vision=[[0.6, -0.2],[0.6, 0.2], [2.6, 1.2], [2.6, -1.2]] #область видимости
        self.vision_global=[] #область видимости в глобальной СК
    def get_pos(self): return (self.x, self.y)
    def filter_visible_pts(self, pts):
        return [p for p in pts if pt_inside_ngon(p, self.vision_global)]
    def draw(self, screen, SCALE, SHIFT):
        ...
        for i in range(-1, len(self.vision)-1):
            pygame.draw.line(screen, (0,0,255), tr(self.vision_global[i]), tr(self.vision_global[i+1]), 1)
    def sim(self, dt):
        ...
        if len(self.traj)==0 or dist(self.get_pos(), self.traj[-1])>0.1: self.traj.append(self.get_pos())
        self.vision_global=[np.add(self.get_pos(), v) for v in rot_arr(self.vision, self.alpha)]
