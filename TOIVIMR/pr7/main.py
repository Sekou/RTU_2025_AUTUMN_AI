...
for ch in test:
    ...
    inp = inp
    #out=net.predict(np.array([inp])) #DENSE
    #out=net.predict(np.array([[inp]])) #LSTM mae
    out=net.predict(np.array([[inp]])) #LSTM categorical_crossentropy
...
for i in range(100):
    ...
    #out=net.predict(np.array([inp])) #DENSE
    #out=net.predict(np.array([[inp]])) #LSTM
    out=net.predict(np.array([[inp]])) #LSTM categorical_crossentropy

    out_ch=train_net.decode_letter_prob(out, alphabet)
